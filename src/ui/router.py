...
class Router:
    ...
    def navigate_rodadas(self, id_torneio: int, rodada: int):
        ...
        self.container.controls.clear()
        view = RodadasView(self, self.page,
                           id_torneio=id_torneio, rodada=rodada).build()
        self.container.controls.append(view)
    ...
    def navigate_admin_rodadas(self, id_torneio: int, rodada: int):
        self._rodada_atual = rodada
        self.container.controls.clear()
        view = AdminRodadasView(self, self.page,
                                id_torneio=id_torneio, rodada=rodada).build()
        self.container.controls.append(view)
        self.page.update()

    def show_admin_rodadas(self):
        """Chamado pelo main após autenticação do admin."""
        if not self.torneio_ativo:
            return
        self.container.controls.clear()
        view = AdminRodadasView(
            self, self.page,
    ...
    def show_time_view(self, time_obj):
        ...
        view = tela_detalhes_time(time_obj, stats, jogadores,
                                   lambda: self.navigate("times"))
        self.container.controls.append(view)
        self.page.update()

    def show_torneio_view(self, torneio_obj):
        self.container.controls.clear()
        view = tela_detalhes_torneio(torneio_obj,
                                      lambda: self.navigate("classificacao"))
        self.container.controls.append(view)
        self.page.update()
